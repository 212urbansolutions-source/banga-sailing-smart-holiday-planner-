module.exports = async function handler(request, response) {
  ...
  if (request.method === "GET") {
    return response.status(200).json({
      ...
      message: "Boatello route planner API is available. Use POST from the planner form to generate a route.",
...
- Do not exceed 700 words total.`,
    ...
    if (error.name === "TimeoutError" || error.name === "AbortError") {
      return response.status(504).json({
        error: "OpenAI route generation timed out. Please try again with fewer days or try once more.",
      });
    }
